		...
		<?php foreach($topArticles as $article): ?>
			<article>
				<header>
					...
					<h2><a href="<?= \yii\helpers\Url::to(['/article/view', 'slug' => $article->slug]) ?>"><?= $article->title ?></a></h2>
				</header>
			</article>
		<?php endforeach; ?>
	</div>
			...
			<?php foreach($worthArticles as $article): ?>
				<article class="cf">
					...
					<header>
						...
						<a href="<?= \yii\helpers\Url::to(['/article/view', 'slug' => $article->slug]) ?>" class="read-btn"><?= Yii::t('app', 'read') ?></a>
					</header>
				</article>
			<?php endforeach; ?>
		</div>
